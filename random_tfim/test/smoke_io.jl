    ...
    mktempdir() do directory
        for mode in ("demo", "full"), boundary in (:periodic, :open)
            ...
            h5open(output, "r") do file
                ...
                @test read(group["pair_counts"]) == (boundary == :open ? collect(16:-1:8) : fill(16, 9))
                pairs = read(group["pair_logC"])
                @test size(pairs) == (16, 9, 2)
                @test boundary == :open ? all(isnan, pairs[10:end, 9, :]) : all(isfinite, pairs)
