...
for (i, L) in enumerate(DIST_L)
    ...
    h5open(joinpath(RESULT_DIR, "L_$L.h5"), "r") do f
        ...
        for (j, T) in enumerate(DIST_T)
            ...
            # The logarithmic time axis excludes t=0; the linear y axis retains all estimates.
            lag = 1:maxlag
            ys = correlation[2:end]
...
Label(fig[5, 1:2], "1,000 disorder realizations · shading: ±1 SEM · heat-bath dynamics", fontsize=16)
...
open(joinpath(OUT_DIR, "plot_notes.txt"), "w") do io
    ...
    println(io, "The time axis is logarithmic and the correlation axis is linear. Only t=0 is omitted; nonpositive means and full SEM bands are retained.")
    ...
    println(io, "Selected temperatures match the run_004 plots for comparison; T=1.02 is not assumed to be the run_005 heat-capacity maximum.")
